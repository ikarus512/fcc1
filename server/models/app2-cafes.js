...
{ // options
  ...
  // capped: 100000, // this option does not allow document updates of growing size
                    ...
                    // http://www.solidsyntax.be/2016/03/26/MongoDB-Cannot-change-the-size-of-a-document-in-a-capped-collection/
  ...
  .then( function(cafes) {
    ...
    .map( function(cafe) {
      ...
      newCafe.timeslots = cafe.timeslots.map( function(timeslot) {
        ...
        newTimeslot.planned = timeslot.users.some( function(user) { return user.equals(userId); });
...
CafeSchema.statics.unplanTimeslot = function(args) {
  ...
  .then( function(cafe) {
    ...
    if (!isfound) throw new PublicError('No timeslot started at '+startTime.toISOString());
